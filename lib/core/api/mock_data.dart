...
class GeneradorMock {
  static List<InteraccionMock> obtenerLikesRecibidos() {
    ...
    return [
      InteraccionMock(usuarioId: ids[0], timestamp: DateTime.now().subtract(const Duration(minutes: 10))),
      InteraccionMock(usuarioId: ids[1], timestamp: DateTime.now().subtract(const Duration(hours: 2))),
      InteraccionMock(usuarioId: ids[2], timestamp: DateTime.now().subtract(const Duration(days: 1))),
      InteraccionMock(usuarioId: ids[3], timestamp: DateTime.now().subtract(const Duration(days: 3))),
    ];
  }
  ...
  static List<InteraccionMock> obtenerVisitas() {
    final ids = _mockUsuarios.map((u) => u.uuid.value).toList();
    return [
      InteraccionMock(usuarioId: ids[4], timestamp: DateTime.now().subtract(const Duration(minutes: 5))),
      InteraccionMock(usuarioId: ids[0], timestamp: DateTime.now().subtract(const Duration(hours: 1))),
      InteraccionMock(usuarioId: ids[3], timestamp: DateTime.now().subtract(const Duration(days: 1))),
    ];
  }
  ...
  static List<InteraccionMock> obtenerMisLikes() {
    final ids = _mockUsuarios.map((u) => u.uuid.value).toList();
    return [
      InteraccionMock(usuarioId: ids[0], timestamp: DateTime.now().subtract(const Duration(days: 2))),
      InteraccionMock(usuarioId: ids[4], timestamp: DateTime.now().subtract(const Duration(days: 1))),
    ];
  }
  static Future<void> sembrarSiVacio(AppDatabase db) async {
    ...
    final mockExistentes = todos.where((u) => idsMock.contains(u.uuid)).toList();
    ...
    if (!hayPropio) {
      ...
      for (final n in noMock) {
        await (db.delete(db.usuarios)..where((u) => u.uuid.equals(n.uuid))).go();
      }
    }
  ...
  static Future<Usuario> crearUsuarioPropio(AppDatabase db, String nombre) async {
    final id = _uuid.v4();
    ...
    return (await (db.select(db.usuarios)..where((u) => u.uuid.equals(id))).getSingle());
  }
  ...
  static Future<void> sembrarConversacionesSiVacio(
    ...
    Future<void> mensaje(
        String otro, String contenido, Duration atras, {required bool esMio}) async {
    ...
    if (otros.isNotEmpty) {
      ...
      await mensaje(otros[0], '¡Hola! Vi que te gusta el café 😄',
          const Duration(days: 2, hours: 20), esMio: false);
      await mensaje(otros[0], '¡Hola! Sí, soy adicto jaja',
          const Duration(days: 2, hours: 18), esMio: true);
      await mensaje(otros[0], '¿Cuál es tu lugar favorito de la ciudad?',
          const Duration(days: 1, hours: 5), esMio: false);
      await mensaje(otros[0], 'El de la esquina de 23, sin dudas',
          const Duration(days: 1, hours: 3), esMio: true);
      await mensaje(otros[0], '¡Buena elección! Deberíamos ir algún día ☕',
          const Duration(hours: 5), esMio: false);
    }
    if (otros.length > 1) {
  ...
  static final _mockUsuarios = [
    UsuariosCompanion.insert(
      ...
      genero: 'mujer',
      buscaGenero: 'hombre',
      biografia: Value('Amante del caf\u00e9 y los libros. Me encanta perderme en una buena historia y descubrir nuevos lugares para leer.'),
      intereses: Value(<String>['\u2764\ufe0f Leer', '\u2601\ufe0f Caf\u00e9', '\u2708\ufe0f Viajar', '\ud83c\udfac Cine', '\ud83c\udfb6 M\u00fasica']),
      altura: Value('1.65'),
      educacion: Value('Universidad'),
      trabajo: Value('Dise\u00f1adora gr\u00e1fica'),
      bebe: Value('no'),
    ...
    UsuariosCompanion.insert(
      ...
      genero: 'hombre',
      buscaGenero: 'mujer',
      biografia: Value('Fot\u00f3grafo y viajero. Siempre buscando el mejor atardecer. Si te gusta la aventura, tenemos algo de qu\u00e9 hablar.'),
      intereses: Value(<String>['\ud83d\udcf7 Fotograf\u00eda', '\u2708\ufe0f Viajar', '\u26bd F\u00fatbol', '\ud83c\udfc3 Running', '\ud83c\udfb8 Guitarra']),
      altura: Value('1.80'),
      educacion: Value('Universidad'),
    ...
    UsuariosCompanion.insert(
      ...
      genero: 'mujer',
      buscaGenero: 'otro',
      biografia: Value('M\u00fasica y cine independiente. Toco el ukelele y veo pel\u00edculas de culto. Busco compa\u00f1\u00edas para conciertos y debates nocturnos.'),
      intereses: Value(<String>['\ud83c\udfb6 M\u00fasica', '\ud83c\udfac Cine', '\ud83c\udfa8 Arte', '\ud83e\udde0 Filosof\u00eda', '\u2615 Caf\u00e9']),
      altura: Value('1.58'),
    ...
    UsuariosCompanion.insert(
      ...
      genero: 'hombre',
      buscaGenero: 'mujer',
      biografia: Value('Chef aficionado y amante del buen vino. Los domingos los dedico a cocinar recetas nuevas. \u00bfTe animas a ser mi catadora?'),
      intereses: Value(<String>['\ud83c\udf7d\ufe0f Cocina', '\ud83c\udf77 Vino', '\u2708\ufe0f Viajar', '\ud83c\udfc4 Surf', '\ud83d\udcda Libros']),
      altura: Value('1.75'),
      educacion: Value('Universidad'),
    ...
    UsuariosCompanion.insert(
      ...
      genero: 'mujer',
      buscaGenero: 'hombre',
      biografia: Value('Yoga y meditaci\u00f3n son mi estilo de vida. Creo en el equilibrio y la conexi\u00f3n genuina. Busco a alguien que disfrute tanto el silencio como la charla.'),
      intereses: Value(<String>['\ud83e\uddd8 Yoga', '\ud83c\udf31 Naturaleza', '\u2615 Caf\u00e9', '\ud83d\udcda Libros', '\ud83c\udfa8 Pintura']),
      altura: Value('1.70'),
